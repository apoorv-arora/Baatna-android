...
public class FABUnit extends ImageButton {
	...
	public FABUnit(Context context) {
		this(context, null);
	}

	public FABUnit(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public FABUnit(Context context, AttributeSet attrs, int defStyle) {
	...
	private void updateCircleSize() {
		mCircleSize = getDimension(mSize == SIZE_NORMAL ? R.dimen.fab_size_primary : R.dimen.fab_size_secondary);
	}
	...
	public void setTitle(String title) {
		...
		if (label != null) {
			label.setTextColor(getResources().getColor(R.color.font_color));
			label.setText(title);
		}
	}
	...
	void updateBackground() {
		...
		if (getId() == R.id.fab_post_request) {
			...
			layerDrawable = new LayerDrawable(new Drawable[] { getResources().getDrawable(mSize == SIZE_NORMAL ? R.drawable.fab_bg_normal : R.drawable.fab_bg_mini), createFillDrawable(strokeWidth), createOuterStrokeDrawable(strokeWidth),
					getIconDrawable() });
	...
	private Drawable createInnerStrokesDrawable(final int color, float strokeWidth) {
		...
		shapeDrawable.setShaderFactory(new ShaderFactory() {
			...
			public Shader resize(int width, int height) {
				return new LinearGradient(width / 2, 0, width / 2, height, new int[] { topStrokeColor, topStrokeColorHalfTransparent, color, bottomStrokeColorHalfTransparent, bottomStrokeColor }, new float[] { 0f, 0.2f, 0.5f, 0.8f, 1f },
						TileMode.CLAMP);
			}
		});
