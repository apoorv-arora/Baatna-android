...
public class FABControl extends ViewGroup {
	...
	public FABControl(Context context) {
		this(context, null);
	}

	public FABControl(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public FABControl(Context context, AttributeSet attrs, int defStyle) {
	...
	public void addButton(FABUnit button) {
		if(button != null){
			try{
				if (indexOfChild(button) != -1  || (findViewById(button.getId()) != null && indexOfChild(findViewById(button.getId())) != -1))
					return;
			}
			catch(Exception e){
			}
	...
	public void removeButton(FABUnit button) {
	...
	private void createLabels() {
		...
		for (int i = 0; i < mButtonsCount; i++) {
			FABUnit button = (FABUnit) getChildAt(i);
			String title = button.getTitle();

			if (button == mAddButton || title == null || button.getTag(R.id.fab_label) != null)
				continue;

			TextView label = new TextViewRegular(context);
			if(CommonLib.isAndroidL())
				label.setElevation(getResources().getDimension(R.dimen.fab_elevation));
			
			label.setText(button.getTitle());

			final FABUnit buttonFinal = button;
			label.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					buttonFinal.performClick();
				}
			});
	...
	public void toggle() {
		...
		if (mListener != null && mListener instanceof Home) {
			// ktodo
			// if (((Home)mListener).animationRunning)
			// return;
		}
