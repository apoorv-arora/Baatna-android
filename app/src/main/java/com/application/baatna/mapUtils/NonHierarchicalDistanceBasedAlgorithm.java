...
public class NonHierarchicalDistanceBasedAlgorithm<T extends ClusterItem> implements Algorithm<T> {
	public static int MAX_DISTANCE_AT_ZOOM = 60; // essentially 100 dp.
	...
	public Set<? extends Cluster<T>> getClusters(double zoom) {
		...
		synchronized (mQuadTree) {
			for (QuadItem<T> candidate : mItems) {
				...
				Bounds searchBounds = createBoundsFromSpan(candidate.getPoint(), discreteZoom);
	...
	private Bounds createBoundsFromSpan(Point p, int span) {
		// TODO: Use a span that takes into account the visual size of the marker, not just its
		// LatLng.

		double halfSpan = getSpanForZoomLevel(span);
	...
	private double getSpanForZoomLevel(int zoom){
		double zoomSpecificSpan = MAX_DISTANCE_AT_ZOOM;
		zoomSpecificSpan = zoomSpecificSpan / Math.pow(2, getZoomSpan(zoom));
		zoomSpecificSpan /= 256;
		return zoomSpecificSpan;

	}

	private int getZoomSpan(int zoom){
	...
	@Override
	public void setMaxZoomDistance(int distance) {
		MAX_DISTANCE_AT_ZOOM = distance;
	}
}
