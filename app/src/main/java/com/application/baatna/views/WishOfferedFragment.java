...
public class WishOfferedFragment extends Fragment implements UploadManagerCallback {
	...
	private class GetWishes extends AsyncTask<Object, Void, Object> {
		...
		protected Object doInBackground(Object... params) {
			try {
				...
				String url = "";
				url = CommonLib.SERVER + "wish/get?type="+WISH_OFFERED+"&start=0&count=" + count;
	...
	public class WishesAdapter extends ArrayAdapter<Wish> {
		...
		public View getView(int position, View v, ViewGroup parent) {
			...
			String title = mContext.getResources().getString(R.string.wish_title_offered_hint) + wish.getTitle().toUpperCase();
			...
			ClickableSpan cs1 = new ClickableSpan() {
				...
				public void updateDrawState(TextPaint ds) {
					...
					ds.setColor(getResources().getColor(R.color.bt_orange_2));
				}
			};
	...
	private class LoadModeWishes extends AsyncTask<Integer, Void, Object> {
		...
		protected Object doInBackground(Integer... params) {
			...
			try {
				CommonLib.ZLog("API RESPONSER", "CALLING GET WRAPPER");
				String url = "";
				url = CommonLib.SERVER + "wish/get?type="+WISH_OFFERED+"&start="+start+"&count=" + count;
				Object info = RequestWrapper.RequestHttp(url, RequestWrapper.WISH_LIST, RequestWrapper.FAV);
				CommonLib.ZLog("url", url);
				return info;
